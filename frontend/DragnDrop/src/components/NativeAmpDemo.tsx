 ...
 * Uses browser-native APIs for audio processing and integrates with DAW state management
 */
...
const NativeAmpDemo: FC = () => {
  ...
  // Load DAW state on component mount
  useEffect(() => {
    ...
    dawAPI.getDawById(SEEDED_DAW_ID)
      .then((daw: DawDTO) => {
        ...
        console.log('LOADED DAW STATE - Structure:', {
          ...
          userId: daw.userId,
          name: daw.name,
          configCount: daw.listOfConfigs?.length || 0,
          configs: daw.listOfConfigs?.map(config => ({
            id: config.id,
            name: config.name,
            componentsCount: config.components?.length || 0,
            firstComponent: config.components?.[0] ? {
              name: config.components[0].name,
              type: config.components[0].type,
              parameters: config.components[0].settings?.parameters
            } : null
          }))
        });
        ...
        if (firstConfig?.components && firstConfig.components.length > 0) {
          // Handle both componentChain and components for compatibility
          const components = (firstConfig as any).componentChain || firstConfig.components;
          console.log('APPLYING LOADED STATE TO UI:', {
            componentsFound: components.length,
            firstComponent: components[0] ? {
              name: components[0].name,
              parameters: components[0].settings?.parameters
            } : null
          });
          ...
          if (success) {
            console.log('Successfully applied preset from DAW state to UI');
            ...
            console.warn('Failed to apply preset from DAW state');
          }
        } else {
          console.log('No components found in first config, using default values');
        }
      })
      .catch((error) => {
        console.error('Error loading DAW state:', error);
      });
  }, []);
  ...
  const handleSavePreset = async () => {
    ...
    try {
      console.log('BUILDING DAW DTO FROM CURRENT STATE');
      console.log('Current amp state:', {
        inputGain,
        directMode,
        distortionAmount,
        bassValue,
      ...
      console.log('DAW DTO CREATED (before sending)');
      console.log('Full DTO:', dawDTO);
      console.log('DTO Summary:', {
        dawId: dawDTO.dawId,
        name: dawDTO.name,
      ...
      console.log('SAVE COMPLETE - Final state');
      console.log('Saved DAW object:', saved);
      console.log('State comparison:', {
        ...
        received: {
          dawId: saved.dawId,
          name: saved.name,
          componentsCount: saved.listOfConfigs?.[0]?.components?.length
        }
      });
      ...
      alert('Failed to save preset. Make sure the backend POST endpoint is implemented.');
    }
  };
  ...
  const handleUpdateDaw = async () => {
    ...
    try {
      ...
      console.log('Full DTO:', updatedDTO);
      ...
      console.log('State comparison:', {
        before: {
          dawId: dawState.dawId,
          componentsParams: dawState.listOfConfigs?.[0]?.components?.[0]?.settings?.parameters
        },
        after: {
          dawId: updated.dawId,
          componentsParams: updated.listOfConfigs?.[0]?.components?.[0]?.settings?.parameters
        }
      });
      ...
      alert('Failed to update preset. Make sure the backend PUT endpoint is implemented.');
    }
  };
    ...
    <div style={{ 
      ...
      {isOn && !directMode && (
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#2a2a2a', borderRadius: '5px' }}>
          ...
          {/* EQ Section */}
          <div style={{ marginBottom: '15px', paddingTop: '15px', borderTop: '1px solid #444' }}>
          ...
          {/* Master Volume */}
          <div style={{ paddingTop: '15px', borderTop: '1px solid #444' }}>
            <label style={{ display: 'block', marginBottom: '5px' }}>
      ...
      <div style={{ 
        ...
        <ol>
          ...
          <li>Click <strong>Save Preset</strong> to save your configuration</li>
        </ol>
