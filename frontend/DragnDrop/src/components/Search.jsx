...
// Hardcoded DAWs + ratings for initial search functionality
const projects = [
    {
        id: 1,
        dawId: 2183908290381,
        ...
        listOfConfigs: [
            { name: 'Default', componentChain: [{ name: 'Chorus' }, { name: 'Delay' }, { name: 'Overdrive' }] }
        ],
        ratings: [
            { userId: 'omar', rating: 5, comment: 'Great vibe' },
            { userId: 'alice', rating: 4, comment: 'Nice textures' }
        ]
    },
    {
        id: 2,
        ...
        listOfConfigs: [
            { name: 'Pad-Long', componentChain: [{ name: 'Reverb' }, { name: 'Phaser' }] }
        ],
        ratings: [
            { userId: 'bob', rating: 3, comment: 'Good, needs work' }
        ]
    },
    {
        id: 3,
        ...
        listOfConfigs: [
            { name: 'Crunch', componentChain: [{ name: 'Overdrive' }, { name: 'Delay' }] }
        ],
        ratings: []
    }
];
...
export default function Search() {
    ...
    useEffect(() => {
        // read persisted user (set by Login)
        try {
        ...
        window.addEventListener('dawker:login', handler);
        return () => window.removeEventListener('dawker:login', handler);
    }, []);
